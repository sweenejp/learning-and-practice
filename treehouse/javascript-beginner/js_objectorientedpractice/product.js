...
const addInventory = (product, amount) => {
    product.inventory += amount;
    console.log(`${amount} ${product.name} added to this inventory. Current inventory is ${product.inventory}`);
};
...
const processSale = (product, amount) => {
    product.inventory -= amount
    console.log(`${amount} ${product.name} sold.`)
...
console.log(`Total sale: ${processSale(cheese, 5)}`);
